...
# docker build -t neutronics_workflow .

# To build this Dockerfile and use multiple cores to compile:
# docker build -t neutronics_workflow --build-arg compile_cores=7 .

# To run the resulting Docker image:
# docker run -it neutronics_workflow

# Run with the following command for a jupyter notebook interface
# docker run -p 8888:8888 neutronics_workflow /bin/bash -c "jupyter notebook --notebook-dir=/examples --ip='*' --port=8888 --no-browser --allow-root"
...
# # Clone and install Embree which is needed for double down
# RUN apt-get update
# RUN apt-get install libembree-dev
# RUN apt-get install -y libembree3-3
